...
const userCtrl = {
  ...
  addUser: async (req, res) => {
    try {
      // Check if the current user is an admin (only admins can add users)
      if (req.user.role !== "Admin") {
        return res.status(401).json({
          title: "Insufficient permissions",
      ...
      const { full_name, email, password, avatar, mobile, address, role } = req.body;
      ...
      const userResponse = await Users.findById(newUser._id).select("-password");
